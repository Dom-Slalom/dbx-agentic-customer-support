...
generate_config = {
    ...
    "all": dbutils.widgets.get("generate_all") == "True",
...
def get_table_count(table_name: str) -> int:
    ...
    return spark.sql(f"SELECT COUNT(*) FROM {catalog}.{schema}.{table_name}").collect()[0][0]
...
if should_generate("plans"):
    ...
    product_gen.save_to_delta(plans_df, f"{catalog}.{schema}.plans")
else:
...
if should_generate("devices"):
    ...
    product_gen.save_to_delta(devices_df, f"{catalog}.{schema}.devices")
else:
...
if should_generate("promotions"):
    ...
    product_gen.save_to_delta(promotions_df, f"{catalog}.{schema}.promotions")
else:
...
if should_generate("customers"):
    ...
    customer_gen.save_to_delta(customers_df, f"{catalog}.{schema}.customers")
else:
...
if should_generate("subscriptions"):
    ...
    customer_gen.save_to_delta(subscriptions_df, f"{catalog}.{schema}.subscriptions")
else:
...
LEFT JOIN {catalog}.{schema}.subscriptions s ON c.customer_id = s.customer_id
...
if should_generate("billing"):
    ...
    billing_gen.save_to_delta(billing_df, f"{catalog}.{schema}.billing")
else:
...
if should_generate("usage"):
    ...
    billing_gen.save_to_delta(usage_df, f"{catalog}.{schema}.usage")
else:
...
FROM {catalog}.{schema}.devices d
JOIN {catalog}.{schema}.subscriptions s ON d.device_id = s.device_id
LEFT JOIN {catalog}.{schema}.promotions p ON s.promo_id = p.promo_id
JOIN {catalog}.{schema}.billing b ON s.subscription_id = b.subscription_id
WHERE
...
FROM {catalog}.{schema}.knowledge_base
